...
struct LightStats {
    double meanLuma = 0.0;          // 平均亮度，范围 0~255
    double darkPixelRatio = 0.0;    // 暗像素比例，范围 0~1
    double brightPixelRatio = 0.0;  // 亮像素比例，范围 0~1
};
...
LightStats computeLightStatsRgb(const RgbFrame& frame) {
    ...
    for (int y = 0; y < frame.height; y += sampleStep) {
        ...
        for (int x = 0; x < frame.width; x += sampleStep) {
            ...
            // RGB -> luma，整数近似：Y ≈ 0.299R + 0.587G + 0.114B
...
LightScene classifyLightSceneCandidate(const LightStats& stats) {
    // 极暗：平均亮度低，同时大部分像素都处于暗区。
    if (stats.meanLuma < 25.0 && stats.darkPixelRatio > 0.75) {
        return LightScene::NIGHT;
    }

    // 明亮：平均亮度高，同时暗像素比例低。
    if (stats.meanLuma > 100.0 && stats.darkPixelRatio < 0.30) {
        return LightScene::DAY;
    }

    // 中间过渡区统一视为 DUSK。这里包含傍晚、室内弱光、局部强光等场景。
    return LightScene::DUSK;
}

void updateLightSceneState(LightSceneState* state,
                           LightScene candidate,
...
double computeTargetVisibleWeight(const LightStats& stats,
                                  LightScene scene,
                                  double maxVisibleWeight) {
    maxVisibleWeight = clampDouble(maxVisibleWeight, 0.0, 1.0);

    if (scene == LightScene::NIGHT) {
        // 方案 A：不关闭可见光 pipeline，但融合时完全不用可见光数据。
        return 0.0;
    }

    if (scene == LightScene::DAY) {
        return maxVisibleWeight;
    }

    // DUSK：根据亮度在 0 ~ maxVisibleWeight 之间平滑变化。
    ...
    // 暗像素比例越高，可见光可信度越低。
    if (stats.darkPixelRatio > 0.70) {
        t *= 0.35;
    } else if (stats.darkPixelRatio > 0.50) {
        t *= 0.65;
    }

    // 少量强光点、整体仍然偏暗时，避免被路灯/车灯误导。
    if (stats.brightPixelRatio > 0.08 && stats.meanLuma < 50.0) {
        t *= 0.60;
    }
...
std::vector<std::uint8_t> fuseRgbResizeNearestAdaptive(
...
int main(int argc, char** argv) {
    ...
    std::cerr << "[CONFIG] composite=" << opt.compositeDevice
              ...
              << "@" << opt.fps << "\n";
    std::cerr << "[CONFIG] fusion=RGB adaptive_visible_weight"
              << " max_visible_weight=" << (1.0 - opt.alpha)
              << " initial_composite_alpha=" << opt.alpha
              << " output=NV12 " << opt.visibleWidth << "x" << opt.visibleHeight
              << " -> mpph264enc -> udp " << opt.host << ":" << opt.port << "\n";
    std::cerr << "[CONFIG] light_state thresholds: "
              << "NIGHT(mean<25,dark>0.75,stable>=3s), "
              << "DAY(mean>100,dark<0.30,stable>=3s), "
              << "DUSK(other stable>=3s); visible pipeline keeps running in NIGHT\n";
    ...
    double visibleWeight = clampDouble(1.0 - opt.alpha, 0.0, 1.0);
    double targetVisibleWeight = visibleWeight;
    ...
    while (!g_stop.load()) {
        ...
        const LightStats visibleLightStats = computeLightStatsRgb(visible);
        const auto nowForLight = std::chrono::steady_clock::now();
        const LightScene candidateScene = classifyLightSceneCandidate(visibleLightStats);
        updateLightSceneState(&lightState, candidateScene, nowForLight);

        targetVisibleWeight = computeTargetVisibleWeight(
            visibleLightStats,
            lightState.current,
            clampDouble(1.0 - opt.alpha, 0.0, 1.0));
        visibleWeight = smoothVisibleWeight(visibleWeight, targetVisibleWeight);

        if ((frameIndex % static_cast<std::uint64_t>(std::max(1, opt.fps))) == 0) {
            std::cerr << "[LIGHT] frame=" << frameIndex
                      << " meanLuma=" << std::fixed << std::setprecision(2)
                      << visibleLightStats.meanLuma
                      << " darkPixelRatio=" << visibleLightStats.darkPixelRatio
                      << " brightPixelRatio=" << visibleLightStats.brightPixelRatio
                      << " scene=" << lightSceneName(lightState.current)
                      << " candidate=" << lightSceneName(lightState.candidate)
                      << " targetVisibleWeight=" << targetVisibleWeight
                      << " visibleWeight=" << visibleWeight
                      << " compositeWeight=" << (1.0 - visibleWeight)
                      << " thresholds={night:mean<25,dark>0.75;day:mean>100,dark<0.30}"
                      << " sampleStep=8"
                      << " visible=" << visible.width << "x" << visible.height
                      << "\n";
        }

        std::vector<std::uint8_t> fusedRgb = fuseRgbResizeNearestAdaptive(visible, composite, visibleWeight);
        if (fusedRgb.empty()) {
        ...
        if (opt.verbose && (frameIndex % static_cast<std::uint64_t>(std::max(1, opt.fps))) == 0) {
            std::cerr << "[FRAME] pushed=" << frameIndex
                      ...
                      << " scene=" << lightSceneName(lightState.current)
                      << " visibleWeight=" << std::fixed << std::setprecision(2) << visibleWeight
                      << " compositeWeight=" << (1.0 - visibleWeight)
                      << "\n";
        }
