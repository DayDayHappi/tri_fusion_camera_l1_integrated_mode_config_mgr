...
Result<void> VisibleCompositeFusion::init(const FusionConfig& config) {
    config_ = config;
...
Result<media::VideoFrame> VisibleCompositeFusion::process(const media::SyncedFrameGroup& input) {
    if (!initialized_) return Result<media::VideoFrame>::error(ErrorCode::NotInitialized, "VisibleCompositeFusion is not initialized");
    ...
    // 空实现：此处后续接入“可见光 + 微光热红外复合画面”真实三光融合算法。
    return Result<media::VideoFrame>::ok(makePlaceholderOutput(aligned.value()));
}

Result<void> VisibleCompositeFusion::setParam(const std::string& key, const std::string& value) {
    if (key.empty()) return Result<void>::error(ErrorCode::InvalidArgument, "fusion param key is empty");
    runtimeParams_[key] = value;
    return Result<void>::success();
}

Result<std::string> VisibleCompositeFusion::getParam(const std::string& key) const {
...
void VisibleCompositeFusion::release() {
    runtimeParams_.clear();
    calibration_.clear();
    initialized_ = false;
}

media::VideoFrame VisibleCompositeFusion::makePlaceholderOutput(const media::SyncedFrameGroup& input) const {
